...
export class MinimalOAuthProvider implements OAuthServerProvider {
	...
	constructor(opts: MinimalOAuthProviderOptions) {
		...
		this.clientsStore = {
			getClient: (id) =>
				id === this.client.client_id ? this.client : undefined,
	...
	async revokeToken(
		...
		const accessEntry = await this.store.get<AccessTokenEntry>(
			"access",
			request.token,
		);
		...
		const refreshEntry = await this.store.get<RefreshTokenEntry>(
			"refresh",
			request.token,
		);
