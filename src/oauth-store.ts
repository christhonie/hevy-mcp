...
export class RedisOAuthStore implements OAuthStore {
	...
	constructor(
		...
		this.redis.on("error", (e) =>
			console.error(`[oauth-store] redis error: ${e.message}`),
		);
		this.redis.on("connect", () =>
			console.error("[oauth-store] redis connected"),
		);
	}
	...
	async set(
		...
		try {
			...
			if (ttlSeconds > 0)
				await this.redis.set(this.k(kind, key), s, "EX", Math.ceil(ttlSeconds));
			else await this.redis.set(this.k(kind, key), s);
		} catch (e) {
			console.error(
				`[oauth-store] set ${kind} failed: ${(e as Error).message}`,
			);
			throw e;
		}
	}
	...
	async get<T>(kind: StoreKind, key: string): Promise<T | undefined> {
		try {
			...
			console.error(
				`[oauth-store] get ${kind} failed: ${(e as Error).message}`,
			);
			return undefined;
		}
	}
	...
	async take<T>(kind: StoreKind, key: string): Promise<T | undefined> {
		try {
			...
			console.error(
				`[oauth-store] take ${kind} failed: ${(e as Error).message}`,
			);
			return undefined;
		}
	}
	...
	async del(kind: StoreKind, key: string): Promise<void> {
		try {
			...
			console.error(
				`[oauth-store] del ${kind} failed: ${(e as Error).message}`,
			);
		}
	}
}
...
export function createOAuthStore(prefix: string): OAuthStore {
	...
	console.error(
		"[oauth-store] REDIS_URL not set — using in-memory store (OAuth state resets on restart)",
	);
